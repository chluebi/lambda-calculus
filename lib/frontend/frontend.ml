...
module Frontend = struct
  ...
  let rec to_string (tree: t) : string =
    match tree with
      ...
      | BinopF (binop, op1, op2) -> (to_string op1) ^ " " ^ (binop_to_string binop) ^ " " ^ (to_string op2)
  ...
  let rec to_v (tree: t) : V.t =
      match tree with 
        ...
        | NumF n -> int_to_v n
        | BinopF (binop, op1, op2) -> V.AppV (V.AppV (V.VarV (binop_to_identifier binop), to_v op1), to_v op2)
